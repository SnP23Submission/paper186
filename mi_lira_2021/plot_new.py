...
def load_data(p):
    ...
    """
    global scores_approx, scores_exact, keep
    scores_approx = []
    scores_exact = []
    keep = []
    ...
    for root,ds,_ in os.walk(p):
        for f in ds:
            ...
            # last_epoch = ['0000000100_approx.npy', '0000000100_exact.npy']
            if len(last_epoch) == 0: continue
            # scores.append(np.load(os.path.join(root,f,"scores",last_epoch[-1])))
            scores_exact.append(np.load(os.path.join(root,f,"scores",last_epoch[-1])))
            scores_approx.append(np.load(os.path.join(root,f,"scores",last_epoch[0])))
            keep.append(np.load(os.path.join(root,f,"keep.npy")))

    # scores_exact.append(np.load(os.path.join(root,f,"scores","0000000100_exact.npy")))
    # scores_approx.append(np.load(os.path.join(root,f,"scores", "0000000100_approx.npy")))
    # keep.append(np.load(os.path.join(root,f,"keep.npy")))
    
    scores_exact = np.array(scores_exact)
    scores_approx = np.array(scores_approx)
    keep = np.array(keep)[:,:scores_exact.shape[1]]

    return scores_exact, scores_approx, keep
...
def do_plot(fn, keep, scores_exact, scores_approx, ntest, legend='', metric='auc', sweep_fn=sweep, **plot_kwargs):
    """
    ...
    path = '/content/drive/MyDrive/msi/MIA/mi_lira_2021/exp/cifar10/tpr_fpr_auc_acc'
    if not os.path.exists(path):
      os.mkdir(path)
    
    np.save(f'{path}/fpr.npy', fpr)
    np.save(f'{path}/tpr.npy', tpr)
    np.save(f'{path}/auc.npy', auc)
    np.save(f'{path}/acc.npy', acc)
...
def fig_fpr_tpr():
    ...
    do_plot(generate_ours,
            keep, scores_exact, scores_approx, 1,
            "Ours (online)\n",
            metric='auc'
    )
